...
@drawio{https://github.com/Edisheri/system-engineering-playbook/blob/main/diagrams-codes/IDEF0_A4.drawio}

### Декомпозиция A4

@drawio{https://github.com/Edisheri/system-engineering-playbook/blob/main/diagrams-codes/IDEF0_A4_DECOMPOSITION.drawio}
...
flowchart TB
    subgraph Inputs["Входы (I)"]
        I1["I1: JSON-данные с результатами"]
        I2["I2: Метаданные пациента"]
        I3["I3: Heatmap изображения"]
    end
    
    subgraph Controls["Управление (C)"]
        C1["C1: Шаблоны отчётов"]
        C2["C2: Требования к формату"]
        C3["C3: Политики конфиденциальности"]
    end
    
    subgraph Function["Функция A4"]
        AA4["Формирование отчёта<br/><br/>A4"]
    end
    
    subgraph Outputs["Выходы (O)"]
        O1["O1: PDF-отчёты"]
        O2["O2: HTML-отчёты"]
        O3["O3: Данные для MIS"]
        O4["O4: Уведомления (WebSocket)"]
    end
    
    subgraph Mechanisms["Механизмы (M)"]
        M1["M1: PDFKit"]
        M2["M2: REST API Client"]
        M3["M3: WebSocket Server"]
        M4["M4: Template Engine"]
    end
    ...
    style AA4 fill:#4a90e2,stroke:#2e5c8a,stroke-width:3px,color:#fff
