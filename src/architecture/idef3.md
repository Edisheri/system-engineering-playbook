...
- После UOB-3: XOR junction 
  ...
  - **Ошибка валидации** → UOB-7 (Обработка ошибки) → UOB-8 (Возврат ошибки пользователю) → Конец процесса
- После UOB-4: AND junction (параллельное выполнение UOB-5 и UOB-6)
- После UOB-5 и UOB-6: Синхронизация → UOB-9 (Подтверждение успешной загрузки) → Конец процесса
...
- После UOB-5 и UOB-6: AND junction 
  - **Оба успешны** → UOB-7 (Агрегация результатов)
  - **Ошибка в одном** → UOB-10 (Обработка ошибки inference) → UOB-11 (Возврат ошибки) → Конец процесса
- После UOB-7: AND junction (параллельное сохранение в Redis и PostgreSQL)
- После UOB-8 и UOB-9: Синхронизация → UOB-12 (Подтверждение успешной обработки) → Конец процесса
...
**Junction:**
- После UOB-4: AND junction (параллельная визуализация UOB-5 и генерация алертов UOB-6)
- После UOB-6: OR junction 
  - **Критическая ошибка обнаружена** → UOB-7 (Отправка алертов DevOps)
  - **Нет критических ошибок** → Продолжение мониторинга
